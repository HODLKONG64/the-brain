...
# Article link text length thresholds for HTML scrape heuristic
MIN_ARTICLE_LINK_TEXT = 10
MAX_ARTICLE_LINK_TEXT = 200

# Minimum consecutive significant words required for a title→wiki match
...
def _parse_rss_feed(feed_url: str) -> list:
    ...
    try:
        ...
        # RSS 2.0
        for item in soup.find_all("item")[:MAX_ARTICLES_PER_SOURCE]:
        ...
        # Atom
        if not items:
            for entry in soup.find_all("entry")[:MAX_ARTICLES_PER_SOURCE]:
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if parsed.netloc != base_domain:
                ...
                if not (
                    base_domain == "substack.com" or
                    base_domain.endswith(".substack.com") or
                    base_domain == "medium.com" or
                    base_domain.endswith(".medium.com")
                ):
                    continue
            ...
            text_ok = MIN_ARTICLE_LINK_TEXT < len(text) < MAX_ARTICLE_LINK_TEXT
...
def _is_in_wiki(article: dict, wiki_text_lower: str) -> bool:
    ...
    # 2. Title match — require at least MIN_TITLE_MATCH_WORDS significant words to match
    if title:
        norm_title = _normalise(title)
        words = [w for w in norm_title.split() if len(w) > 3]
        if len(words) >= MIN_TITLE_MATCH_WORDS:
            # Check if at least MIN_TITLE_MATCH_WORDS consecutive words appear in wiki
            for i in range(len(words) - MIN_TITLE_MATCH_WORDS + 1):
                phrase = " ".join(words[i:i + MIN_TITLE_MATCH_WORDS])
...
def _build_queue_entry(article: dict, update_type: str, source_label: str) -> dict:
    ...
    if not ts:
        ts = _format_utc_timestamp(datetime.datetime.now(datetime.timezone.utc))
    else:
        # Try to normalise the timestamp to ISO format
        try:
            from email.utils import parsedate_to_datetime
            dt = parsedate_to_datetime(ts)
            ts = _format_utc_timestamp(dt.astimezone(datetime.timezone.utc))
        except Exception:
            pass
...
def _format_utc_timestamp(dt: "datetime.datetime") -> str:
    """Return a UTC ISO-8601 timestamp string ending in 'Z'."""
...
def detect_wiki_gaps(dry_run: bool = False, verbose: bool = False) -> dict:
    ...
    run_ts = _format_utc_timestamp(
        datetime.datetime.now(datetime.timezone.utc)
    )

    print("[wiki-gap-detector] 🔍 Wiki Gap Detector starting…")
    ...
    if wiki_text:
        ...
        print("[wiki-gap-detector] ⚠️  Wiki page unavailable — all articles flagged as gaps")
