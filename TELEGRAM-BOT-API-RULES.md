...
- Bulk broadcasts to >30 different chats: max **30 messages/second** total.
...
- For channels with >100 members: Telegram may throttle faster than 1/second.
...
| File size (sendDocument, sendPhoto etc.) | 50 MB via Bot API |
| Photo file size | 10 MB |
| Animation/video file size via Bot API | 50 MB |
...
- Bots operating in channels/groups must comply with Telegram's Terms of Service and any applicable local laws.
- NSFW/adult content is only permitted in age-gated channels that have been explicitly approved by Telegram.
- Automated bots must identify themselves as bots — they must not impersonate real humans in ways that deceive users.
...
| Edit history | ✅ | ✅ | ❌ |
| Message threading | ❌ | ✅ (with topics enabled) | ❌ |
...
- Max **20 replies per user per 24 hours** (GK BRAIN internal limit — enforced by `reply-tracker.json`). This is stricter than Telegram's own limits, which is intentional to prevent spam reports.
